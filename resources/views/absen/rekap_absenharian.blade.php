    ...
    table th {
        ...
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ route('rekap_absenharian.index') }}" method="GET">
                    @foreach($tapels as $tapel)
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @foreach ($students as $student)
                                @php
                                    $absenByMonth = $student->absenharian->groupBy(function ($item) {
                ...
                <div id="rekap_bulan_table_{{ $tapel->id }}" style="display: none; margin-top: 20px;">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @foreach ($students as $student)
                                @php
                                    $filteredAbsens = $student->absenharian->filter(function ($absen) use ($selectedBulan) {
                                        return \Carbon\Carbon::parse($absen->waktu_absen)->format('m') == $selectedBulan;
                                    });
                                @endphp
                                <tr>
                                    <td>{{ $student->nama }}</td>
                                    <td>{{ $student->nisn . '/' . $student->nuptk }}</td>
