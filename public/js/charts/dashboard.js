(function (jQuery) {
    ...
    if (document.querySelectorAll("#myChart").length) {
        ...
        const presentase = JSON.parse(el.dataset.presentase);
        const options = {
            series: presentase, // Misalnya persentase hadir dan tidak hadir
            chart: {
                type: "donut",
                height: 230,
            },
            colors: ["#1aa053", "#f0ad4e", "#0275d8", "#fa003f", "#babfbd"],
            plotOptions: {
                pie: {
                    donut: {
                        size: "65%", // ukuran lubang tengah
                        labels: {
                            show: false, // Hapus label di tengah donut
                        },
                    },
                },
            },
            dataLabels: {
                enabled: false, // Tidak menampilkan label angka di luar donut
            },
            legend: {
                show: false, // Jika ingin legend juga disembunyikan
            },
            labels: ["Hadir", "Izin", "Sakit", "Alfa", "Belum Absen"], // Bisa diganti sesuai kebutuhan
        };
    ...
    if (document.querySelectorAll("#d-main").length) {
        ...
        const cost = JSON.parse(el.dataset.cost);
        const labels = JSON.parse(el.dataset.labels);
        console.log(sales, cost, labels);
        const options = {
            series: [
                {
                    name: "Hadir",
